...
class Cptm
{
	...
	function __construct()
	{
		...
		// could not understand the first init method call, check after all method inspections ==> it's a hidden post type for this plugin
		add_action( 'init', array($this, 'cptm_create_custom_post_types') );
		// register cpt and cpt taxonomy and integrate into admin menu
		add_action( 'admin_menu', array($this, 'cptm_admin_menu') );
		// register plugin interface into admin menu
		add_action( 'admin_enqueue_scripts', array($this, 'cptm_styles') );
		add_action( 'add_meta_boxes', array($this, 'cptm_create_meta_boxes') );
		add_action( 'add_meta_boxes', array($this, 'create_meta_if_checked'));
		add_action( 'save_post', array($this, 'save_meta_if_checked'));
	...
	/**
	 * Initialize plugin
	 * It's two custom post type (cpt+tax) and creates another/multiple real custom post type
	 * and taxonomy
	 */
	 ...
	 * ==> It creates real custom post type and taxonomy and handshake bitween them
	 */
	public function cptm_create_custom_post_types()
	{
		...
		if ( $cptm_post_types ) {
			foreach ( $cptm_post_types as $cptm ) {
				...
				$price = unserialize( $cptm_supports );
				//print_r($price);
				//$price_field_checked = array_search('price-field', $price ) ? true : false;
				//echo $price_field_checked ? 'exists' : 'not checked';
	...
	public function create_meta_if_checked($post){
		//add metabox or custom field for created post type
		// check if checkbox checkbox value is exists
		// then add the metabax through custom types loops
		$get_cptm = array(
			'numberposts' 	   => -1,
			'post_type' 	   => 'cptm',
			'post_status'      => 'publish',
		...
		if ( $cptm_post_types ) {
			foreach ( $cptm_post_types as $cptm ) {
				$cptm_meta = get_post_meta( $cptm->ID, '', true );
				$price =( array_key_exists( 'cptm_supports', $cptm_meta ) && $cptm_meta['cptm_supports'][0] ) ? unserialize($cptm_meta['cptm_supports'][0]) : false;
				if($price) {
					if(array_search('price-field', $price )){
						add_meta_box(
							'price_field',
							__( 'Price', 'custom-post-type-maker' ),
							array($this, 'price_field_meta_box'),
							$cptm_meta['cptm_name'][0],
							'advanced',
							'high'
						);
					}
	...
	*/
	...
	public function price_field_meta_box($post){
		$value = get_post_meta($post->ID, 'price_field', true);
		echo '<lable for="price-field">Price</lable><input type="text" value="'.$value.'" name="price_field" />';
	}
	...
	public function cptm_meta_box( $post ) {
		...
		$cptm_supports_price_field         = ( isset( $values['cptm_supports'] ) && in_array( 'price-field', $cptm_supports ) ? 'price-field' : '' );
		...
		<table class="cptm">
			...
			<tr>
				...
				<td>
					...
					<input type="checkbox" tabindex="33" name="cptm_supports[]" id="cptm_supports_price_field" value="price-field" <?php checked( $cptm_supports_price_field, 'price-field' ); ?> /> <label for="cptm_supports_price_field"><?php _e( 'Price Custom Field', 'custom-post-type-maker' ); ?></label><br />
